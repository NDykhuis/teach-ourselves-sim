...
#from studentwatcher import studentwatcher
...
class simulation:
    ...
    def setup(self):
        ...
        # Initialize the student watchers
        #self.swatch = [studentwatcher()
    
        if cfg.outputdata:
            if not self.dt:
                dt = strftime("%Y-%m-%d_%H:%M:%S")
            self.datafile = open('data_'+self.dt+'.dat','w')  # for simple dumping of raw data
            self.datafile.write("(\n")
        if cfg.outputagents:
            self.dumpagents()
